...
# ====================== 全局配置 ======================
APP_PACKAGE_NAME = "com.oymotion.eeghelmet"
...
def test_switch_all_views(device_driver):
    ...
    # 右上角菜单按钮（保留你原来的写法）
    menu_btn = device_driver.xpath('//*[@content-desc="查看波形"]/following-sibling::android.widget.Button[1]')
    ...
    # ---------------------- 切换 IMU视图 ----------------------
    menu_btn.click()
    device_driver(description="IMU").wait(timeout=SLEEP_DEFAULT)
    device_driver(description="IMU").click()
    print("✅ 切换到：IMU视图")

    # ---------------------- 切换 FFT视图 ----------------------
    menu_btn = device_driver.xpath('//*[@content-desc="IMU"]/following-sibling::android.widget.Button[1]')
    assert menu_btn.wait(timeout=WAIT_TIMEOUT_NORMAL), "❌ 未找到菜单按钮"
    menu_btn.click()
    device_driver(description="FFT").wait(timeout=SLEEP_DEFAULT)
    device_driver(description="FFT").click()
    print("✅ 切换到：FFT视图")

    # ---------------------- 切换 直方图 ----------------------
    menu_btn = device_driver.xpath('//*[@content-desc="FFT"]/following-sibling::android.widget.Button[1]')
    assert menu_btn.wait(timeout=WAIT_TIMEOUT_NORMAL), "❌ 未找到菜单按钮"
    menu_btn.click()
    device_driver(description="PowerBand").wait(timeout=SLEEP_DEFAULT)
    device_driver(description="PowerBand").click()
    print("✅ 切换到：直方图")

    # ---------------------- 切回 波形图 ----------------------
    menu_btn = device_driver.xpath('//*[@content-desc="PowerBand"]/following-sibling::android.widget.Button[1]')
    assert menu_btn.wait(timeout=WAIT_TIMEOUT_NORMAL), "❌ 未找到菜单按钮"
    menu_btn.click()
    device_driver(description="Waveform").wait(timeout=SLEEP_DEFAULT)
    device_driver(description="Waveform").click()
    print("✅ 切回默认：波形图")
...
def test_recovery_default_freq(device_driver):
    """波形页面-恢复默认频率"""
    recovery_default_freq = device_driver.xpath('//android.widget.Button[6]')
...
#     """波形页面-暂停/开始"""
#     pause_resume = device_driver.xpath('//android.widget.Button[7]')
