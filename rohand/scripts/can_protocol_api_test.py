...
from rohand.api.OHandSerialAPI import (
    HAND_RESP_SUCCESS, HAND_PROTOCOL_UART,
    MAX_MOTOR_CNT, MAX_THUMB_ROOT_POS, MAX_FORCE_ENTRIES,
    OHandSerialAPI
)
...
DELAY_MS = 1600
DELAY_MS_FUN = 5000
DELAY_MS_DEVICE_REBOOT = 10000
SKIP_CASE = False

# 全局配置（外部调用可修改）
CLIENT_PORT = "1"
CLIENT_BAUDRATE = 1000000
...
def serial_api_instance():
    """
    统一初始化夹具，所有测试用例共享
    设备ID 固定使用全局变量 HAND_ID
    """
    can_interface_instance = None
    serial_api_instance = None

    try:
        logger.info(f"初始化 CAN 接口: port={CLIENT_PORT}, baudrate={CLIENT_BAUDRATE}")
        can_interface_instance = CAN_Init(port_name=CLIENT_PORT, baudrate=CLIENT_BAUDRATE)
        if can_interface_instance is None:
            logger.error("CAN 端口初始化失败")
            yield None
            return

        protocol = HAND_PROTOCOL_UART
        serial_api_instance = OHandSerialAPI(
            can_interface_instance,
            protocol,
            ADDRESS_MASTER,
            send_data_impl,
            recv_data_impl
...
def main(ports: list = [], devices_ids: list = [], aging_duration: float = 1.5):
    ...
    full_port = ports[0] if len(ports) > 0 else "PCAN_USBBUS1"

    # 给驱动使用的端口：只取最后一位数字
    CLIENT_PORT = full_port[-1]
